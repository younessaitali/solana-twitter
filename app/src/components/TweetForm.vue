...
const send = async () => {
  console.log(canTweet.value);

  if (!canTweet.value) return;
  console.log(
    "🚀 ~ file: TweetForm.vue ~ line 52 ~ send ~ effectiveTopic.value",
    effectiveTopic.value
  );
  console.log(
    "🚀 ~ file: TweetForm.vue ~ line 53 ~ send ~  content.value",
    content.value
  );
  const tweet = await sendTweet(effectiveTopic.value, content.value);
  console.log("🚀 ~ file: TweetForm.vue ~ line 54 ~ send ~ tweet", tweet);
  emit("added", tweet);
  topic.value = "";
  content.value = "";
};
const setTopic = (event: Event) => {
